<template>
  <div>
    ...
    <el-col :span="24">
      <!--  上传组件  -->
      <strong v-for="(i,index) in numsInfo">{{i.value}}：{{i.key}}</strong>
      <!--   时间排序模块   -->
    ...
    <el-col :span="24">
      <videoListAcc :infos="picInfos" :total="count" @remove="removeData" @pageChange="pageChange"></videoListAcc>
    </el-col>
    <!--  弹出框内容  -->
    <el-dialog
        title="上传该视频"
...
<script>
  ...
  import videoListAcc from "../../../components/video/videoListAcc";
  import UpLoadPop from "../../material/pop/upLoadPop";
  import dialogsPop from "../../material/pop/dialogsPop";

  export default {
    name: "videoDetails",
    components: {UpLoadPop, videoListAcc, uploadFile, dialogsPop},
    props: {
      member_id: String,
      required: true
    },
    ...
    methods: {
      async getImgs(limit, page,) {
        this.picInfos = []
        this.$api.memberVideo({
            limit,
            page,
          ...
          .then((res) => {
            ...
            this.numsInfo[0].key = this.count
            console.log(this.numsInfo[0].key)
          })
        // .catch((error) => {
